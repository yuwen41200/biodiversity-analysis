...
class DataProximity:
    def __init__(self, dataset, percentage=0.1):
        ...
        :param percentage: The number of records taken into account when calculating species ranking
        """
    ...
    def extractDataset(self):
        ...
        return [(k, self.temporalData[k][i][0].timestamp(), u[0])
                for k, coords in self.spatialData.items()
                for i, u in enumerate(coords)]

    def recordRank(self, dataset, number=0, sortByDistance=True):
        """
        ...
        :return: List of pairs of Data, where "Data" is tuple of (time difference, distance, index
        of first element in dataset, index of second element in dataset)
        """
    ...
    def speciesRank(self, onlyTake=0, temporalWeight=0.5, spatialWeight=0.5):
        ...
        :return: List of tuples of Data, where "Data" is a tuple of (score, (species1,species2))
        """
